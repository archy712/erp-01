...
- [x] 이메일/비밀번호 및 구글 OAuth 로그인/회원가입/로그아웃 정상 동작 — 이메일 회원가입(2계정)/로그인/로그아웃을 Playwright로 직접 재현. 구글 OAuth는 Task 009와 동일한 경계로 "Google로 계속하기" → `accounts.google.com` 실제 리다이렉트까지 재확인(동의 화면 완주는 실계정 2FA가 필요해 범위 밖, 기존 결정 유지).
- [x] 다크모드 토글 즉시 반영 + 새로고침 후 유지 — `/erp/menu/...` 화면에서 다크→라이트 전환 즉시 반영 확인, `/erp`로 새로고침 후에도 라이트 유지 확인.
- [x] `archy712@gmail.com` 계정이 관리자 권한 보유 — `execute_sql`로 `role='superadmin'` 재확인.
- [x] 메뉴 관리에서 대/중/소분류 등록·수정·정렬 가능 + 대분류 단독 등록 가능 — 이번 Task에서 대분류 단독 등록("Task022 검증용 대분류")을 직접 재현해 Menubar/트리에 즉시 반영됨을 확인(등록/수정/삭제/정렬 자체는 Task 016에서 이미 전수 검증됨, 이번엔 대분류 단독 등록 경로만 재확인).
- [x] 사용자 권한 관리에서 임의 레벨 메뉴 부여/회수 가능 — 소분류("법인 관리")와 대분류 단독 2건("웹POS", "Task022 검증용 대분류")을 한 번에 부여 → `execute_sql`로 정확히 3건 저장 확인(상위 노드는 indeterminate라 저장 제외되는 기존 설계 재확인).
- [x] 일반 사용자는 권한 있는 메뉴만 노출 + URL 직접 접근 차단 — 부여받은 3개 대분류만 Menubar에 노출(나머지 12개 숨김), 권한 없는 소분류("브랜드 관리") 및 관리자 전용 URL(`/erp/admin/users`) 직접 접근 시 각각 `AccessDenied`/`  /erp/forbidden` 확인.
- [x] 관리자는 전체 메뉴(마스터 관리 + 14개 Placeholder) 접근 가능 — 관리자 로그인 시 15개 대분류 전체가 Menubar에 노출됨을 재확인(전수 클릭 검증은 Task 020에서 이미 완료).
- [x] 기본 관리 3종을 제외한 모든 메뉴가 공용 빈 화면으로 정상 렌더링 — "법인 관리"로 대표 확인(breadcrumb + `MenuPlaceholder` + "추후 구현 예정" Badge), 전수 검증은 Task 019에서 이미 완료.
- [x] 전체 메뉴 트리(마스터 관리 하위 3개 그룹 + 14개 Placeholder)가 DB에 모두 등록됨 — `execute_sql`로 `level` 그룹별 카운트(대분류 15/중분류 3/소분류 12 = 30) 재확인.
- [x] `mcp__supabase__get_advisors`로 보안/성능 경고 최종 확인 및 해소 — security 9건(SECURITY DEFINER 함수 관련, `set_user_menu_permissions` 포함) + performance 3건(전부 `weekly_log_*`/`departments` 등 weeklyplan 쪽 테이블) 전부 Task 011~020에서 이미 검토되어 수용된 기존 경고이며 신규 경고 없음.
- [x] `npm run check-all` 및 `npm run build` 통과 — 둘 다 신규 에러 없음(lint는 기존 warning 7건만 유지).

**(검증 중 발견한 관찰 사항 — 수정하지 않음)** 모바일(390px)에서 로그인 사용자 이메일이 길 경우(`erp-task022-user@example.com` 등) 헤더의 인사말·프로필·로그아웃 버튼이 줄바꿈되며 서로 겹치는 현상을 발견했다. 원인을 추적한 결과 스타터킷 원본 `app/page.tsx`의 `AuthButton` 마크업(`"Hey, {email}!"` 패턴, 줄바꿈/말줄임 처리 없음)에서부터 있던 특성이며, 이번 세션에서 추가한 다국어 인사말(`dict.common.greeting`)이 새로 만든 문제가 아니다. `docs/roadmap/ROADMAP_MVP.md`의 "범위 제외" 목록에 "기존 Header/Footer 재설계"가 명시되어 있고 Header 마크업은 "디자인 변경 없음" 원칙으로 복제된 것이라 이번 Task에서는 수정하지 않고 관찰 기록만 남긴다. 콘솔 에러는 0건이라 기능적 문제는 없음.

**테스트 체크리스트 (Playwright MCP) — 시나리오별 E2E**

임시 테스트 계정 2개(`erp-task022-admin@example.com` / `erp-task022-user@example.com`, 회원가입 직후 관리자 계정만 SQL로 `role='admin'` 승격)와 임시 대분류 1개("Task022 검증용 대분류")로 검증 후 전부 삭제(`execute_sql`로 `menus`=30건/`profiles`=0건/`user_menu_permissions`=0건 재확인). 시딩된 운영 데이터(마스터 관리 16건 + Placeholder 14건)는 건드리지 않음.

- [x] **시나리오 A (관리자)**: 로그인 → 대분류 단독 등록(즉시 Menubar 반영 확인) → 사용자 권한 부여(소분류 1 + 대분류 단독 2, indeterminate 상위 노드 확인) → 로그아웃
- [x] **시나리오 B (일반 사용자)**: 로그인 → 부여받은 3개 대분류만 Menubar 노출 확인(나머지 12개 숨김) → "마스터 관리 > 기준정보 관리 > 법인 관리" 트리 진입(형제 5개 숨김 확인) → breadcrumb+제목 확인 → 로그아웃
- [x] **시나리오 C (권한 없는 접근)**: 권한 없는 소분류("브랜드 관리") URL 직접 입력 → `AccessDenied` 확인, 관리자 전용 URL(`/erp/admin/users`) 직접 입력 → `/erp/forbidden` 리다이렉트 → "홈으로 돌아가기"로 복귀 확인
- [x] **시나리오 D (반응형)**: 시나리오 B를 1440(정상) / 768(트리 폭 축소+Menubar 유지) / 390px(햄버거+Sheet, 그룹 펼침→Sheet 유지→리프 선택→Sheet 닫힘+내비게이션)에서 각각 재현, 스크린샷으로 확인
- [x] **시나리오 E (구글 OAuth)**: "Google로 계속하기" 클릭 → `accounts.google.com` 실제 리다이렉트 확인(Task 009와 동일 경계 — 동의 화면 완주 및 권한 부여 재확인 사이클은 실계정 2FA 필요로 범위 밖 유지)
- [x] `browser_console_messages`로 전 시나리오 콘솔 에러 0건 확인
