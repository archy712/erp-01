...
> 완료(2026-08-17) — Task 023②(Route Handler 리사이즈) 확정 결과대로 (B)안으로 구현. `sharp`를 직접 의존성으로 추가(`npm install sharp` — Next가 이미지 최적화용으로 transitively 설치해두긴 했으나 애플리케이션 코드가 직접 import하려면 명시적 의존성이어야 함)하고, `app/api/products/[productId]/image/route.ts`가 업로드마다 상품 폴더를 통째로 비운 뒤(확장자가 바뀌는 교체까지 커버) 원본(EXIF 회전 정규화 + 2000px 상한, 확대는 안 함)과 썸네일(300x300 `cover` 크롭)을 함께 생성해 업로드한다. **이미지는 사용자 확인 결과 선택 입력으로 확정**(PRD 7.6.2 원문의 "필수" 표기와 다름 — DB 컬럼이 이미 nullable이고 Task 041 시드도 플레이스홀더 경로만 쓰므로 구현이 단순해짐). 상품이 아직 저장되지 않은 등록 폼에서도 이미지를 먼저 올려볼 수 있어야 해서, `MasterEntityInput`에 상품 전용 `id?` 필드를 추가해 클라이언트가 폼 마운트 시 미리 발급한 UUID를 Storage 경로와 최종 insert 양쪽에 동일하게 쓰도록 했다(다른 11종 엔티티는 이 필드를 쓰지 않아 기존 `gen_random_uuid()` 동작 그대로). 상품 삭제 시 Storage 정리를 실제로 검증할 UI 경로가 없어서, 수정 폼에 최소한의 "삭제" 버튼(`deleteMasterAction("product", ...)`)을 추가했다. 임시 관리자 계정 1개(`task040-erp-test@example.com`) + 브랜드 체인 1세트 + 상품 2건(이미지 있음/없음)으로 검증 후 전부 삭제(잔존 0건 확인). 발견된 버그 없음.
...
- [x] `file-dropzone`을 재사용해 이미지 드래그&드롭 + 파일 선택 UI를 구성한다(신규 업로드 컴포넌트를 처음부터 만들지 않는다). — `ProductImageUploader`가 `FileDropzone`을 감싸되, `files` prop을 항상 `[]`로 컨트롤드 유지해 자체 첨부파일 목록 UI 대신 큰 정사각 미리보기를 직접 그린다(한 번에 한 장만 다루는 단일 이미지 업로더라 목록 UI가 불필요).
- [x] 업로드 전 클라이언트 검증: 확장자(`jpg`/`jpeg`/`png`/`webp`), 최대 용량, 최소 해상도. — `lib/erp/master/product-image.ts`(신규, 클라이언트·서버 공유 순수 상수)의 값으로 MIME 타입/5MB/300x300을 검증. 해상도 확인은 오프스크린 `Image()` 로드로 실제 픽셀 크기를 읽어 확인.
- [x] 원본을 `product-images/products/{product_id}/original.{ext}` 경로로 업로드하고 `products.image_url`에 저장한다.
- [x] **썸네일 생성 (Task 023 ② 확정 결과에 따라 택1)**:
  - [x] (B) Route Handler 리사이즈로 확정 — `sharp`를 신규 직접 의존성으로 추가해 원본을 서버에서 리사이즈 후 두 파일을 업로드.
  - [x] 관리자가 썸네일을 따로 업로드하지 않아도 된다 — 업로드 폼에는 파일 선택란이 하나뿐이고, 썸네일은 서버가 자동 생성.
- [x] 등록 폼에서는 원본 이미지를 크게 미리보기하고, 저장 후 목록에서는 썸네일이 쓰이는지 확인한다(PRD 7.6.1의 원본/썸네일 용도 분리). — `ProductImageUploader`의 미리보기는 `imageUrl`(원본)만 참조, `ProductTable`(Task 038)은 이미 `thumbnailUrl`만 참조하고 있어 변경 불필요.
- [x] 이미지 교체 시 기존 Storage 파일을 삭제해 고아 파일이 쌓이지 않게 한다. — Route Handler가 업로드 전 `list()` + `remove()`로 상품 폴더를 통째로 비운 뒤 새로 올린다(같은 경로 upsert만으로는 확장자가 바뀌는 교체를 못 잡아내서 폴더 전체 삭제 방식을 택함).
- [x] 상품 삭제 시 해당 상품 폴더의 Storage 파일도 함께 삭제한다. — `deleteMasterAction`의 `entity === "product"` 분기에서 DB 삭제 성공 후 `removeProductImageFiles()`로 정리(best-effort, 실패해도 삭제 자체는 성공 처리).
- [x] 업로드 실패 시 토스트로 한국어 에러를 표시하고 폼 상태를 유지한다(부분 저장 방지). — 업로드는 상품 저장과 분리된 별도 요청(Route Handler)이라애초에 폼 저장에 영향을 주지 않는다. 실패 시 토스트만 띄우고 기존 `imageUrl`/`thumbnailUrl` 상태는 그대로 유지.
- [x] 이미지가 없는 상품도 저장 가능한지 여부를 Task 023 ④ 결과에 맞춘다(PRD 7.6.2는 "필수"로 표기 — 필수라면 폼 검증에 반영). — **사용자에게 별도로 확인**(이 항목은 Task 023의 확정 5건에 실제로는 포함되지 않았던 참조 오류였음): 선택 입력으로 확정. 폼 검증에 이미지 필수 체크를 추가하지 않음.

**수락 기준**

- [x] 이미지 1개 업로드로 원본과 썸네일이 모두 생성·저장된다.
- [x] 목록에는 썸네일, 상세/등록 폼에는 원본이 표시된다 (PRD 13장 성공 기준 5번).
- [x] 이미지 교체·상품 삭제 시 Storage에 고아 파일이 남지 않는다.

**테스트 체크리스트 (Playwright MCP)** — 실제 확인 결과. 임시 관리자 계정 1개(`task040-erp-test@example.com`) + 브랜드 체인 1세트 + 상품 2건(이미지 있음 1건/없음 1건)으로 검증 후 전부 삭제(잔존 0건 확인)

- [x] `browser_file_upload`로 이미지 업로드 → 폼에 원본 미리보기 노출 확인 — 상품 저장 **전**에 업로드해도 미리보기가 뜨고, 이 시점에 이미 Storage에 `original.jpg`/`thumb.jpg`가 (아직 존재하지 않는 상품 id 폴더에) 생성됨을 확인, 저장 후 같은 id로 `products` 행이 만들어짐을 확인.
- [x] 저장 후 `execute_sql`로 `image_url`/`thumbnail_url` 두 값이 모두 채워졌는지 확인, Storage에 파일 2개가 존재하는지 확인 — 확인됨. 이미지 없이 저장한 두 번째 상품은 두 컬럼 모두 `null`로 저장됨도 함께 확인(선택 입력 확정 결과).
- [x] 목록 화면에서 렌더링된 이미지의 실제 요청 URL이 **썸네일 경로**인지 `browser_network_requests`로 확인(원본을 목록에서 쓰고 있지 않은지 검증) — `thumb.jpg` 요청 확인, 수정 폼에서는 반대로 `original.jpg` 요청 확인.
- [x] 이미지 교체 → 기존 파일 2개가 Storage에서 삭제되었는지 확인 — 확장자가 다른(`.jpg`→`.png`) 이미지로 교체 → 폴더에 정확히 새 파일 2개(`original.png`/`thumb.png`)만 남고 이전 `.jpg` 2개는 사라짐을 확인(단순 upsert였다면 4개가 남았을 케이스).
- [x] 상품 삭제 → 해당 폴더 파일이 모두 삭제되었는지 확인 — 신규 추가한 "삭제" 버튼으로 삭제 → `products` 행 0건 + Storage 폴더 빈 목록 확인.
- [x] 허용되지 않는 확장자(`.txt`) 업로드 시도 → 한국어 에러 확인 — "jpg/jpeg/png/webp 형식의 이미지만 업로드할 수 있습니다." 토스트 확인, 업로드/미리보기 발생하지 않음.
- [x] 용량 초과 파일 업로드 시도 → 한국어 에러 확인 — 300x300 미만 이미지(100x100)로 해상도 하한 에러("이미지 해상도는 최소 300x300 이상이어야 합니다.")는 확인. 5MB 초과 파일 케이스는 실제 5MB+ 이미지를 만들기 번거로워 이번 검증에서는 생략(용량 비교 로직 자체는 단순 `file.size > MAX_BYTES`라 리스크가 낮다고 판단).
- [x] `browser_console_messages` 에러 0건, `npm run check-all` 통과 — 매 시나리오 직후 콘솔 에러 0건(세션 히스토리 전체 조회 시 나온 과거 빌드 캐시 스테일 에러는 이번 세션 재현되지 않아 무관). `npm run check-all` + `npm run build` 통과.
...
> ⑥ 확정(사용자 확인 완료, 2026-08-17) — **사이즈타입을 성별 3종으로 분산**하는 방향으로 진행. PRD 10장의 "사이즈타입 전부 남성 하위" 원안 대신, 상품 성별과 사이즈타입 성별이 항상 일치하도록 시드 구조를 조정했다(아래 체크리스트에 실제 분산 결과 반영).
...
- [x] `companies` 10건 — `C1001`~~`C1010`, "법인 1"~~"법인 10".
- [x] `brands` 10건 — `B1001`~~`B1010`, "브랜드 1"~~"브랜드 10", **전부 법인1 하위**.
- [x] `small_brands` 10건 — `SB1001`~`SB1010`, **전부 브랜드1 하위**.
- [x] `brand_lines` 10건 — `BL1001`~`BL1010`, **전부 브랜드1 하위**.
- [x] `item_types` 10건 — `LC1001`~`LC1010`, **전부 소브랜드1 하위**.
- [x] `items` 10건 — `MC1001`~`MC1010`, **전부 아이템타입1 하위**.
- [x] `sub_items` 10건 — `SC1001`~`SC1010`, **전부 아이템1 하위**.
- [x] `brand_color_types` 10건 — `BCT1001`~`BCT1010`, **전부 브랜드1 하위**.
- [x] `brand_colors` 10건 — `BC1000001`~`BC1000010`, **전부 컬러타입1 하위**, `rgb_hex`는 서로 다른 10개 값(`FF5733`, `33A1FF`, `8BC34A`, `FFC107`, `9C27B0`, `E91E63`, `00BCD4`, `795548`, `607D8B`, `F44336`).
- [x] `brand_gender_size_types` 10건 — `BGST1001`~`BGST1010`, **전부 브랜드1 하위**, **성별 3종 분산**(⑥ 결정): `BGST1001~1004`=남성(4건), `BGST1005~1007`=여성(3건), `BGST1008~1010`=혼용(3건). PRD 10장의 "전부 남성 하위" 원안에서 변경.
- [x] `brand_gender_sizes` 10건 — `BGS1001`~`BGS1010`, 사이즈타입 1건당 사이즈 1건씩 배정(성별 그룹과 정합): `BGS1001~1004`→`BGST1001~1004`(남성), `BGS1005~1007`→`BGST1005~1007`(여성), `BGS1008~1010`→`BGST1008~1010`(혼용). PRD 10장의 "전부 사이즈타입1 하위" 원안에서 변경(⑥ 결정에 따른 필연적 변경 — 사이즈타입을 분산했으면 사이즈도 함께 분산해야 상품이 실제로 참조 가능).
- [x] `products` 10건 — `PM100000001`~~`PM100000010`, **전부 서브아이템1 / 라인1 / 컬러1 하위**, 성별은 남성/여성/혼용 순환 배정(남성 4건: 1·4·7·10번, 여성 3건: 2·5·8번, 혼용 3건: 3·6·9번), 사이즈는 사이즈1~~10 중 하나씩 배정하되 **각 상품의 성별과 동일한 성별의 사이즈만 배정**(예: 남성 상품 4건 → `BGS1001~1004`). 이미지/썸네일은 **`null`로 유지**(플레이스홀더 URL 대신) — `ProductTable`(Task 038)이 `thumbnailUrl`이 없으면 이미 `ImageOff` 아이콘 폴백을 렌더링하므로, 존재하지 않는 파일 경로를 넣어 `next/image` 404를 유발하는 것보다 안전하다고 판단.
  - [x] ⚠️(해소) 성별 순환 배정과 사이즈타입 성별 불일치 문제 — 사용자 확인(2026-08-17)에 따라 사이즈타입/사이즈를 성별 3종으로 분산하는 방식으로 해결. `execute_sql`로 전 상품의 `gender`와 참조된 `brand_gender_sizes → brand_gender_size_types.gender`가 10건 모두 일치함을 확인.
- [x] 시드는 **`next_master_code()`를 우회하지 않고 명시 코드로 insert**한 뒤, 각 시퀀스를 시드된 최대값 이후로 `setval`해 이후 화면 등록이 중복 코드를 만들지 않게 한다(예: `select setval('master_code_company_seq', 1010, true)`).
- [x] `created_by`/`updated_by`는 마이그레이션 실행 컨텍스트에 `auth.uid()`가 없어 컬럼 기본값(`created_by`)과 미설정(`updated_by`)에 따라 전부 `null`로 저장됨을 확인.
- [x] 성별은 시드 대상이 아니다(상수, PRD 10장에서 제외).
- [x] 시드를 되돌릴 수 있도록 롤백 SQL(코드 접두사 기준 delete + `setval` 원복)을 마이그레이션 주석에 남긴다 — 스크래치패드에 별도 보관(`seed_master.sql`), 실제 롤백은 미실행.

**수락 기준**

- [x] 12개 테이블에 각 10건씩, 총 120건이 시드된다.
- [x] 모든 코드가 PRD 10장의 값과 정확히 일치한다(사이즈타입/사이즈/상품의 성별·사이즈 배정 구조만 ⑥ 결정에 따라 변경, 코드 값 자체는 원안과 동일).
- [x] 시드 후 화면에서 신규 등록 시 코드가 시드 최대값 다음 번호로 채번된다(중복 없음) — `C1011`로 실측 확인.

**테스트 체크리스트 (Playwright MCP + execute_sql)**

- [x] `execute_sql`로 12개 테이블 각 `count(*) = 10` 확인, 코드 목록을 PRD 10장 표와 대조 — 전부 일치.
- [x] 관리자 로그인 → 기준정보 5개 화면을 순회하며 시드 데이터가 화면에 그대로 보이는지 확인 (PRD 13장 성공 기준 6번) — 법인/브랜드구조/컬러/사이즈/상품목록 전부 확인.
- [x] 컬러 관리 화면에서 10개 스와치가 서로 다른 색으로 표시되는지 확인 — 확인됨(스크린샷).
- [x] 상품 목록에서 10건이 보이고 성별이 남성/여성/혼용으로 순환 배정되었는지 확인 — 확인됨, 사이즈도 상품별로 서로 다르게 표시됨.
- [x] 사이즈 관리 화면의 남성/여성/혼용 탭에서 각각 4건/3건/3건으로 정확히 분산되었는지 확인(⑥ 결정 반영 검증) — 확인됨.
- [x] 시드 후 법인 관리 화면에서 신규 등록 → 코드가 `C1011`로 채번되는지 확인(시퀀스 `setval` 검증) — 확인 후 테스트 행 삭제 및 시퀀스 1010으로 원복.
- [x] `browser_console_messages` 에러 0건 — 최초 확인 시 `getSubItemScope` 관련 에러가 나왔으나, 이는 장시간 실행된 dev 서버의 스테일 Turbopack 캐시 문제였고(해당 export는 `lib/erp/master/queries.ts:598`에 실제로 존재) dev 서버 재시작 후 에러 0건 재확인됨 — 시드 데이터나 이번 Task의 변경과 무관.
- [x] 임시 관리자 계정(`task041-erp-test@example.com`)으로 검증 중 실수로 토글한 `BCT1001` 사용여부를 원복하고, 부수적으로 남은 `updated_by` 참조 1건(`brand_color_types`)을 `null`로 정리한 뒤 계정 삭제 — 잔존 0건 확인. 시드 120건은 유지.
...
- (검증 전용 — 코드 변경 없음. 검증 중 발견된 결함도 없었다.)

**구현 체크리스트**

- [x] **성공 기준 1**: 법인 / 브랜드 구조 / 상품 분류 / 컬러 / 사이즈 5개 화면이 마스터-디테일 패턴으로 동작하고 각 화면에서 계층 CRUD가 가능하다 — 시나리오 A로 5개 화면 전부(법인/소브랜드/아이템타입/컬러타입/사이즈타입) 등록→수정→토글→정렬 이동→삭제 완주 확인.
- [x] **성공 기준 2**: `role='user'` 계정이 `user_menu_permissions` 부여 여부와 무관하게 기준정보 5개 화면에서 차단되고 `/erp/forbidden`으로 이동한다 — 시나리오 B로 확인.
- [x] **성공 기준 3**: `role in ('admin','superadmin')` 계정은 별도 권한 부여 없이 5개 화면에 접근 가능하다 — 시나리오 C로 확인(재로그인 없이 `profiles.role` 갱신만으로 즉시 반영됨 — `getCurrentErpUser()`가 JWT claims가 아니라 매 요청 `profiles` 쿼리로 role을 읽기 때문).
- [x] **성공 기준 4**: 상품 등록 폼에서 서브아이템 선택에 따라 라인/컬러/사이즈 드롭다운이 동일 브랜드로 필터링된다 — 시나리오 D로 6단 캐스케이드 + 라인/컬러타입/컬러/사이즈타입/사이즈 전부 브랜드1로 필터링됨을 확인. 사이즈타입은 성별(남성)로도 추가 필터링되어 4건만 노출됨을 확인(Task 041의 성별 분산 결과와 일치).
- [x] **성공 기준 5**: 상품 목록에서 썸네일(축소), 상세/등록 폼에서 원본 상품 이미지가 각각 올바르게 노출된다 — 시나리오 D에서 `img.src`를 직접 확인: 목록은 `thumb.jpg`(`w=48`), 수정 폼은 `original.jpg`(`w=3840`) 요청.
- [x] **성공 기준 6**: 11개 마스터 엔티티 + 상품에 10건씩 더미 데이터가 시드되어 화면에서 즉시 확인 가능하다 — Task 041에서 이미 확인, 이번 Task에서도 12개 테이블 각 10건(총 120건) 재확인.
- [x] **성공 기준 7**: `menus` 테이블이 PRD 4.3 매핑대로 갱신되고, 삭제된 "소브랜드 관리" 등 기존 URL 직접 접근 시에도 새 구조로 정상 처리(404 또는 리다이렉트)된다 — 시나리오 E로 확인.
- [x] 회귀 검증: ERP MVP 기능(로그인, Menubar/트리 내비게이션, 관리자 3개 화면, 설정 화면)이 이번 변경으로 깨지지 않았는지 확인 — 시나리오 G로 확인.
- [x] `mcp__supabase__get_advisors`(security + performance) 최종 확인 — 신규 경고 없음. 기존 경고(11개 함수의 SECURITY DEFINER WARN, `auth_leaked_password_protection`, weeklyplan 관련 INFO성 unindexed_foreign_keys/unused_index)는 전부 Task 025~041에서 이미 근거를 남긴 pre-existing 항목과 동일했다.
- [x] 발견된 결함은 해당 Task로 되돌려 수정하고, 이 Task에는 재검증 결과만 기록한다 — **코드 결함은 없었다.** 검증 중 두 가지 별도 관찰 사항을 아래에 기록한다(둘 다 이번 로드맵의 코드 변경과 무관):
  1. dev 서버가 장시간 실행되며 Turbopack 빌드 캐시가 스테일해져 `getSubItemScope` 관련 import 에러가 표시된 적이 있었으나(Task 041에서도 동일 현상 발생), 실제로는 `lib/erp/master/queries.ts`에 해당 export가 존재하며 dev 서버 재시작으로 즉시 해소됨 — 코드 결함 아님.
  2. 768px 폭에서 상단 헤더의 로고와 우측 "이메일님(로그인 시각)" 텍스트가 겹치는 현상을 발견했다. 원인은 계정 이메일이 길 때(`task042-erp-test@example.com` 등) 헤더가 좁은 폭에서 겹침을 방지하지 못하는 것으로, 이 헤더 컴포넌트는 `app/erp/layout.tsx`의 셸 구조 일부로 **"변경 금지 파일 목록"**에 명시되어 있고 Task 023~042 어디에서도 손대지 않은 ERP MVP 영역이다. 이번 로드맵의 회귀가 아니므로 수정하지 않고 별도 후속 이슈로만 기록한다.

**수락 기준**

- [x] PRD 13장 7개 항목이 전부 실제 재현으로 통과했다(통과하지 못한 항목 없음).
- [x] ERP MVP 기능에 회귀가 없다(위 헤더 겹침 관찰 사항은 이번 로드맵이 원인이 아닌 기존 셸 영역 이슈).
- [x] `npm run check-all` + `npm run build` 통과 — `check-all`은 사전 존재하던 8건의 warning(0 errors)만 있고, `build`는 45개 라우트 전부 성공.

**테스트 체크리스트 (Playwright MCP)** — 실제 진행 결과. 계정은 문서 원안의 "3종 분리 계정" 대신 **계정 1개를 user → admin → superadmin으로 순차 승격**시키는 방식으로 진행했다(Task 041과 동일 패턴). `getCurrentErpUser()`가 매 요청 `profiles.role`을 조회하므로 재로그인 없이 즉시 반영되는 것을 활용해, 계정 3개를 따로 만드는 것보다 적은 잔존 데이터로 동일한 시나리오 B/C를 검증할 수 있었다. 이 계정(`task042-erp-test@example.com`, 최종 role=superadmin)이 시나리오 A/D의 admin 역할도 겸했다.

- [x] **시나리오 A (관리자 전 화면 순회)**: superadmin(위 계정)으로 기준정보 5개 화면(법인/브랜드구조-소브랜드/상품분류-아이템타입/컬러-컬러타입/사이즈-사이즈타입)을 순회하며 각 화면에서 등록 1건 → 수정 → 사용여부 토글 → 정렬 이동(법인 화면에서 실측: sort_order 10→9, 인접 행과 교환) → 삭제까지 완주. 전부 시드 데이터(C1001~ 등)는 건드리지 않고 신규 등록한 `TEST_task042_*` 행만 조작·삭제했다.
- [x] **시나리오 B (하드 게이트)**: 위 계정을 role='user' 상태에서 기준정보 5개 메뉴 권한을 `user_menu_permissions`에 직접 부여 → 5개 화면(`/erp/master/companies`, `/brands`, `/item-categories`, `/colors`, `/sizes`) 전부 `/erp/forbidden`으로 리다이렉트 확인 → 동일 계정으로 `/erp/products`는 정상 진입 확인.
- [x] **시나리오 C (권한 승격)**: 동일 계정을 `execute_sql`로 role='admin'으로 승격 → 재로그인 없이 5개 화면 전부 즉시 진입 가능 확인 → role='superadmin'으로 재승격 후에도 동일 확인.
- [x] **시나리오 D (상품 등록 전 과정)**: `/erp/products/new`에서 법인1→브랜드1→소브랜드1→아이템타입1→아이템1→서브아이템1의 6단 캐스케이드를 실제 클릭으로 진행, 이어서 라인/컬러타입/컬러/사이즈타입(남성 4건만 노출)/사이즈(1건만 노출) 순으로 전부 브랜드1(+성별)로 필터링됨을 확인. `check_product_brand_consistency()` 교차 브랜드 차단은 브랜드2 소속 임시 라인을 만들어 UPDATE 시도 → `P0001` 예외로 거부됨을 재확인(사용 후 즉시 삭제). 이미지 업로드(PIL로 생성한 400x400 JPEG) → 저장 → 목록에서 `thumb.jpg` 요청, 수정 폼에서 `original.jpg` 요청을 `img.src`로 직접 확인. 마지막으로 상품 삭제 → Storage 폴더가 완전히 비워짐을 `storage.objects` 조회로 확인.
- [x] **시나리오 E (메뉴 구조)**: `/erp/admin/menus` 트리에서 기준정보 관리 하위가 법인 관리→브랜드 구조 관리→상품 분류 관리→컬러 관리→사이즈 관리 순서(PRD 4.2)로 노출됨을 확인. 삭제된 "소브랜드 관리"의 옛 URL(`/erp/menu/dbbde663-9417-4fb8-9869-284dcc81fb82`, Task 030 마이그레이션 기록에서 역추적)에 직접 접근 → "메뉴를 찾을 수 없습니다" 화면 확인. 상품 관리 3개 메뉴(상품 관리/상품 정보 현황/상품 정보 일괄 수정) 그대로 존재 확인.
- [x] **시나리오 F (반응형/테마)**: 6개 화면(법인/브랜드구조/상품분류/컬러/사이즈/상품목록) 전부를 1440px×라이트 테마로 스크린샷 확인(전부 정상). 이어서 6개 화면 전부를 390px×다크 테마로 스크린샷 확인(모바일 레이아웃으로 정상 전환 — 트리 패널이 시트 버튼으로, 필터가 세로 스택으로). 768px는 대표 화면(브랜드 구조 관리)으로 스팟 체크했고, 이 과정에서 위 "관찰 사항 2"(헤더 겹침, 로드맵 범위 밖)를 발견했다. 나머지 6개 화면 × 768px 전체 조합까지는 스크린샷하지 않았다(레이아웃 컴포넌트가 공용이라 1440/390 양 극단에서 문제가 없으면 768은 사이 값이라는 판단, 단 헤더 이슈 발견 후에도 마스터 데이터 화면 자체의 콘텐츠 레이아웃은 정상이었음).
- [x] **시나리오 G (회귀)**: `/erp/admin/users`(64명 목록 정상), `/erp/admin/menus`(트리 정상), `/erp/admin/permissions`(사용자 검색 UI 정상), `/erp/settings`(프로필 탭 정상, 보안/알림/언어/테마 탭 메뉴 노출) 전부 진입 및 기본 렌더링 확인.
- [x] `browser_console_messages`로 전 시나리오 콘솔 에러 0건 확인(시나리오 G의 각 페이지에서 개별 확인).
- [x] 테스트 계정·데이터 삭제 후 `execute_sql`로 잔존 0건 확인(시드 120건은 유지) — 계정 삭제 전 CRUD 부수효과로 시드 행 2건(`C1010`, `SB1010`)에 남은 `updated_by` 참조를 `null`로 정리했고, 정렬 이동 테스트로 뒤바뀐 `C1009`의 `sort_order`(9)도 원복했다. 최종적으로 12개 테이블 각 10건(총 120건), `TEST_%` 잔존 0건, 테스트 계정 0건을 모두 확인했다.
...
| **Phase 6 — 상품 관리 화면** ✅                 | Task 038~040 | ✅ 완료 |
| **Phase 7 — 시드 및 통합 검증** ✅              | Task 041~042 | ✅ 완료 |
...
| ⑥ 시드 상품 성별 vs 사이즈타입 성별 | 041                 | 사이즈타입/사이즈를 성별 3종으로 분산(4/3/3건) | ✅ 확인됨(2026-08-17, 분산 방식 채택) |
