...
- **그룹사(`org_groups`)**: 1건 고정. `GRP0001`, 이름은 **"OO그룹"으로 확정**(2026-08-17, 가안 그대로 채택).
- **법인(`org_companies`)**: 최소 1건. `OC0001`, 이름은 **"OO 법인"으로 확정**(그룹사 하위, 가안 그대로 채택).
...
- **부서(`org_sections`)**: "IT부문" 하위에 **1건으로 확정**(`OS0001` "개발부서"). 나머지 팀은 부서 없이 부문 직속으로 남겨 **"부서가 없을 수도 있다"는 상태를 그대로 보존**한다.
- **부서↔팀 매핑(`org_section_teams`)**: 8개 팀 중 **`ERP시스템팀`/`Commerce시스템팀`/`IT기획팀` 3개로 확정**해 "개발부서"에 연결하고, 나머지 5개 팀(`ISMS-P 프로젝트팀`/`보안 운영지원팀`/`원격관제 프로젝트팀`/`인사/회계 운영지원팀`/`접근제어 프로젝트팀`)은 매핑 없이 부문 직속으로 둔다.
- **조직별 리더(`org_unit_leaders`)**: **전체 미지정으로 시작하는 것으로 확정**(2026-08-17). 그룹사/법인/부문/부서/팀 5개 레벨 전부 이번 더미 데이터 시드에서 리더 행을 넣지 않는다 — 관리 화면(7장)이 완성된 뒤 실사용자가 직접 지정하며, 이 방식이 곧 "리더 미지정" 상태 UI를 모든 레벨에서 자연스럽게 보여준다.
...
| 그룹사/법인/부서 이름, 초기 리더 지정자                                  | **2026-08-17 확정**: 그룹사 "OO그룹"·법인 "OO 법인"(가안 그대로 채택), 부서 "개발부서" 1건(3개 팀 편입), **초기 리더는 전체 미지정으로 시작**(9장)                                              | 확정 완료 — 리더 지정은 관리 화면(7장) 완성 후 실사용자가 직접 수행                                                                                                                                            |
