...
// 유틸리티 바로 축소했다 — 대시보드 메인 화면(/erp)과 설정(/erp/settings/*)
// 화면에서만 예외적으로 홈 화면과 같은 링크 모음을 이 바 안에 함께 보여준다
// (마케팅 푸터 원본은 app/page.tsx의 SiteFooter에 그대로 남아 있다). 설정
// 화면은 관리자 CRUD나 업무 화면과 달리 폼 중심이라 데이터 테이블이 필요로
// 하는 세로 공간 압박이 적어 풀 푸터를 허용해도 무방하다.
export function ErpFooter({ dict }: { dict: Dictionary }) {
  const pathname = usePathname();
  const showFullFooter =
    pathname === "/erp" || pathname?.startsWith("/erp/settings");

  return (
    <footer
      className={cn(
        "flex w-full shrink-0 flex-col items-center justify-center gap-2 border-t px-4 text-xs text-muted-foreground",
        showFullFooter ? "py-3" : "h-10",
      )}
    >
      {showFullFooter && <FooterLinkNav dict={dict} />}
