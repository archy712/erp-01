...
// 상품 폼은 자체 PageHeader + 5개 탭 바를 먼저 그린 뒤 카드 없이 필드가
// 바로 나열되므로(product-form.tsx), 두 형태를 showPageHeader/showTabs로
// 토글한다.
export function ErpFormPageSkeleton({
  ...
  /** 상품 폼처럼 PageHeader 아래 탭 바(분류/속성/이미지/일반/공통)가 있는
   * 화면이면 true. showPageHeader가 false면 무시된다. */
  showTabs = false,
  /** 카드(또는 탭 콘텐츠) 폭 제한 클래스. 설정 화면 각 page.tsx가 쓰는
   * 값과 동일한 기본값을 둔다. */
  ...
  if (showPageHeader) {
    return (
      <div className="flex flex-1 flex-col">
        ...
        {showTabs ? (
          // product-form.tsx의 TabsList 자리(분류/속성/이미지/일반/공통 5개)
